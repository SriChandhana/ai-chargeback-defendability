...
Evidence is classified as **critical** or **supporting** depending on the dispute type — e.g. proof of delivery is critical for `ITEM_NOT_RECEIVED`, while tracking number is only supporting.
...
Full breakdown available in [`eval/EVALUATION_REPORT.md`](eval/EVALUATION_REPORT.md).

### Important Limitation

These 100% metrics should **not** be read as real-world predictive accuracy.

The synthetic ground-truth labels were generated from the same evidence-completeness rule the deterministic scorer checks. So this evaluation primarily confirms the implementation correctly reproduces its own designed rule — it is a correctness check, not proof of real-world judgment quality.

The critical-vs-supporting evidence weighting is a designed heuristic and has not been validated against real historical chargeback outcomes. A stronger future evaluation would use an independently labeled dataset or real dispute resolution history — not a "held-out test set" in the strict sense, since the labels and the scorer share a common origin.
...
**Field-name mismatches.** The evidence-requirement map initially referenced fields (`product_listing_snapshot`, `device_ip_match`, `mandate_log`) that didn't exist anywhere in the actual dataset schema, which only has seven real evidence fields (`has_shipping_confirmation`, `has_tracking_number`, `has_delivery_proof`, `has_invoice_match`, `has_customer_communication`, `has_auth_otp_log`, `has_cancellation_record`). Caught before the scorer was built on top of it, and the map was corrected to reference only real fields.
...
│   ├── disputes.json
│   ├── evaluation_labels.json
│   └── reason_code_evidence_map.json
│
├── scorer/
│   └── defendability_scorer.py
│
├── eval/
│   ├── evaluate.py
│   ├── scorer_output.json
